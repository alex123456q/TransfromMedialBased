...
bool Collinear2(Point *r0, Point *r1, Point *r2)
/*коллинеарность трех собственных точек*/
{
    bool result = false;

    result = abs((r1->X - r0->X) * (r2->Y - r0->Y) - (r1->Y - r0->Y) * (r2->X - r0->X)) < 100;
    std::cout << "bol "<< abs((r1->X - r0->X) * (r2->Y - r0->Y) - (r1->Y - r0->Y) * (r2->X - r0->X)) << std::endl;
    return result;
}
